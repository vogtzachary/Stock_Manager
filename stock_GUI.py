# This file is a GUI form of stock_menu.py.
# Select the Symbol or Ticker when importing csv data.
...
class StockApp:
    def __init__(self):
        ...
        self.webmenu.add_command(label="Import CSV from Yahoo! Finance...", command=self.import_csv_web_data)
            ...
            for stock in self.stock_list:
                if stock.symbol == symbol:
                    ...
                    for daily_data in stock.DataList:
                        ...
                        price_change = low_price - high_price

                    if count > 0:
                        self.stockReport.insert(END, "Summary Data--\n\n")
                        self.stockReport.insert(END, "Low Price: " + "${:,.2f}".format(low_price) + "\n")
                        self.stockReport.insert(END, "High Price: " + "${:,.2f}".format(high_price) + "\n")
                        self.stockReport.insert(END,
                                                "Average Price: " + "${:,.2f}".format(price_total / count) + "\n\n")
                        self.stockReport.insert(END, "Low Volume: " + str(low_volume) + "\n")
                        self.stockReport.insert(END, "High Volume: " + str(high_volume) + "\n")
                        self.stockReport.insert(END,
                                                "Average Volume: " + "${:,.2f}".format(volume_total / count) + "\n\n")
                        self.stockReport.insert(END, "Change in Price: " + "${:,.2f}".format(price_change) + "\n")
                        self.stockReport.insert(END,
                                                "Profit/Loss: " + "${:,.2f}".format(price_change * stock.shares) + "\n")
                    else:
    ...
    def delete_stock(self):
        if self.stockList.curselection():
            ...
            messagebox.showinfo(f"Stock Deleted: {symbol}")
    ...
    # Display Stock Historicals Chart
